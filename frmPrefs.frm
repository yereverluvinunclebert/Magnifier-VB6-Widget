...
'    gblPlhLocationPercPrefValue
    ...
    If gblPlStartup = "1" Then
        Call savestring(HKEY_CURRENT_USER, "SOFTWARE\Microsoft\Windows\CurrentVersion\Run", "magnifierWidget", """" & App.Path & "\" & "magnifier.exe""")
    Else
        Call savestring(HKEY_CURRENT_USER, "SOFTWARE\Microsoft\Windows\CurrentVersion\Run", "magnifierWidget", vbNullString)
    End If
...
Private Sub lblGitHub_dblClick()
    ...
    If answer = vbYes Then
       Call ShellExecute(Me.hwnd, "Open", "https://github.com/yereverluvinunclebert/Magnifier-VB6-Widget", vbNullString, App.Path, 1)
    End If
...
Private Sub sliGaugeSize_Change()
    ...
    btnSave.Enabled = True ' enable the save button
    If gblAllowSizeChangeFlg = True Then magnifierWidget.Zoom = sliGaugeSize.Value / 100
...
loadHigherResPrefsImages_Error:
    ...
    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure loadHigherResPrefsImages of Form magnifierPrefs"
End Sub
'---------------------------------------------------------------------------------------
...
setPrefsIconImagesDark_Error:
    ...
    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure setPrefsIconImagesDark of Form magnifierPrefs"
...
setPrefsIconImagesLight_Error:
    ...
    MsgBox "Error " & Err.Number & " (" & Err.Description & ") in procedure setPrefsIconImagesLight of Form magnifierPrefs"
